 ...
 * group it lives under). Those are preserved exactly as-is.
 *
 * Only genuinely new files (never seen in index.html before) get their
 * name/date/group resolved. For new files the order of precedence is:
 *   1. a <!-- meta: ... --> comment at the top of the HTML file
 *   2. the slug-based guess
 * Files that no longer exist in /proposals are dropped. Hand-edit a
 * name/group once and it sticks on every future run, because this script
...
// Reads a <!-- meta: client=Acme; group=client; title=Partnership Overview; date=2026-07 -->
...
function groupFromMeta(meta) {
  if (!meta) return null;
  const raw = meta.group || meta.venture; // "venture" accepted as an alias
  if (!raw) return null;
  const key = raw.trim().toLowerCase();
  return { key, title: GROUP_TITLES[key] || raw.trim() };
}
...
function guessGroup(slug) {
  const clean = slug.replace(/\/$/, '').toLowerCase();
  for (const key of Object.keys(GROUP_TITLES)) {
    if (key === SITE.fallbackGroup) continue;
    if (clean.startsWith(key + '-')) return { key, title: GROUP_TITLES[key] };
  }
  const fb = SITE.fallbackGroup;
  return { key: fb, title: GROUP_TITLES[fb] || 'Everything Else' };
}

function guessDate(slug) {
  const clean = slug.replace(/\/$/, '');

  // Trailing -MMYY, e.g. client-offerings-grid-0626
  ...
  // Trailing -monYYYY, e.g. client-ic-package-jun2026
...
function guessName(slug) {
  let s = slug.replace(/\/$/, '');
  for (const key of Object.keys(GROUP_TITLES)) {
    if (s.toLowerCase().startsWith(key + '-')) { s = s.slice(key.length + 1); break; }
  }
  s = s.replace(/-\d{4}$/, '');
...
function buildEntries() {
  ...
  const buckets = new Map(); // groupKey -> { title, entries: [] }
  const newlyAdded = [];
  ...
  for (const { href, slug, file } of onDisk) {
    ...
    if (!record) {
      ...
      const venture = groupFromMeta(meta) || guessGroup(slug);
  ...
  // Order the group sections consistently.
  const orderedKeys = [
...
<style>
  :root {
    ...
    --body-text: ${SITE.theme.bodyText};
    --cyan: ${SITE.theme.accent};
    --purple: ${SITE.theme.accentAlt};
    --gradient: linear-gradient(90deg, ${SITE.theme.accent}, ${SITE.theme.accentAlt});
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-lock {
    ...
    color: var(--purple); background: color-mix(in srgb, var(--purple) 9%, transparent);
    border: 1px solid color-mix(in srgb, var(--purple) 22%, transparent); border-radius: 20px; padding: 2px 8px;
  }
...
<div class="wrap">
  ...
  <div class="wordmark">
    ...
    <span class="wordmark-text">${escapeHtml(SITE.brandName)}</span>
  </div>

  <h1>${escapeHtml(SITE.pageTitle)}</h1>
  <p class="sub">${escapeHtml(SITE.tagline)}</p>
...
${ventureBlocks}
  ...
  <footer>
    ${escapeHtml(SITE.repoLabel)} &middot; auto-deployed via Railway
  </footer>
...
function main() {
  ...
  console.log(`index.html regenerated: ${totalCards} document(s) across ${buckets.size} group(s). ${GATED.size} gated.`);
