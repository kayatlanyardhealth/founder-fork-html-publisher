...
//
// e.g. slug "client-partnership-0726" -> GATE_CLIENT_PARTNERSHIP_0726
//
...
try {
  if (fs.existsSync(MANIFEST_PATH)) {
    ...
    console.log('No protected.json found — all documents open.');
  }
} catch (err) {
  manifestBroken = true;
...
// First path segment identifies the document, so directory-style documents
...
function noticePage(heading, message) {
  return `<html>
      <body style="font-family: sans-serif; padding: 2rem; background: #F6F7F9;">
        <h2 style="color: #16181D;">${heading}</h2>
...
// Index route — serves the master index.html listing every live link
app.get('/', (req, res) => {
...
app.use((req, res) => {
  res.status(404).send(`
    <html>
      <body style="font-family: sans-serif; padding: 2rem; background: #F6F7F9;">
        <h2 style="color: #16181D;">Document not found</h2>
...
app.listen(PORT, () => {
  console.log(`HTML publisher running on port ${PORT}`);
});
